...
const CF_DEFS = [
  ["cnpj", "EOL CNPJ"],
  ["razao_social", "EOL Razao Social"],
  ["nome_fantasia", "EOL Nome Fantasia"],
  ["situacao", "EOL Situacao Cadastral"],
  ["cnae_codigo", "EOL CNAE Codigo"],
  ["cnae_descricao", "EOL CNAE Descricao"],
  ["natureza", "EOL Natureza Juridica"],
  ["porte", "EOL Porte"],
  ["capital", "EOL Capital Social"],
  ["municipio", "EOL Municipio"],
  ["uf", "EOL UF"],
  ["data_inicio", "EOL Data Inicio Atividade"],
  ["matriz_filial", "EOL Matriz ou Filial"],
  ["simples", "EOL Optante Simples"],
  ["mei", "EOL Optante MEI"],
  ["socios", "EOL Socios"],
  ["socio_qualificacao", "EOL Qualificacao do Socio"],
  ["origem", "EOL Origem"],
  ["importado_em", "EOL Importado em"],
];
...
async function upsertContact(token, locationId, businessId, contato, cf, usedEmails) {
  ...
  const body = { locationId, firstName: first, lastName: last, source: "Encontre o Lead" };

  let email = (contato.email || "").trim().toLowerCase() || null;
  const phone = contato.phone || null;
  if (email && usedEmails.has(email)) email = null;
  if (!email && !phone) {
    email = `${contato.emailBase}@encontreolead.local`.toLowerCase();
    let i = 2;
    while (usedEmails.has(email)) email = `${contato.emailBase}_${i++}@encontreolead.local`.toLowerCase();
  }
  if (email) {
    body.email = email;
  ...
  body.tags = contato.tags || ["encontreolead"];
